 ...
 * Goal:
 */
public class ReloadableResourceYamlMessageSource extends AbstractMessageSource implements ResourceLoaderAware {
    ...
    private YAMLPropertiesPersister propertiesPersister = new YAMLPropertiesPersister();
    ...
    protected PropertiesHolder refreshProperties(String filename, PropertiesHolder propHolder) {
        ...
        if (!resource.exists()) {
            resource = this.resourceLoader.getResource(filename + YAML_SUFFIX);
        }
    ...
    protected Properties loadProperties(Resource resource, String filename) throws IOException {
        ...
        try {
            if (resource.getFilename().endsWith(YAML_SUFFIX)) {
                if (logger.isDebugEnabled()) {
                    logger.debug("Loading properties [" + resource.getFilename() + "]");
                }
                this.propertiesPersister.loadFromYaml(props, is);
            }
            else {
    ...
    public void clearCacheIncludingAncestors() {
        clearCache();
        if (getParentMessageSource() instanceof ReloadableResourceYamlMessageSource) {
            ((ReloadableResourceYamlMessageSource) getParentMessageSource()).clearCacheIncludingAncestors();
        }
    }
...
class YAMLPropertiesPersister implements PropertiesPersister{
    ...
    public void loadFromYaml(Properties props, InputStream is) throws IOException {
        props.putAll(YamlUtils.yamlToMap(is));
    }
